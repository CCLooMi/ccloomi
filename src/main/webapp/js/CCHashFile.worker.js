...
self.addEventListener('message', function (event) {
    ...
    if(!block.stop){
        ...
        var message = util.bytesToWords(new Uint8Array(event.data.message));
        event = null;
        if (block.end === block.file_size) {
            ...
            output.result = util.bytesToHex(util.wordsToBytes(self.hash));
        } else {
            if(!self.hasFirstHash&&block.step==3){
                //为不影响后面的计算，这里进行数组浅拷贝，（对于多维数组需要深拷贝）
                var copyHash=self.hash.concat();
                output.firstHash=util.bytesToHex(util.wordsToBytes(copyHash));
